...
import { createStatusTransaksi, updateStatusTransaksi } from '@/app/actions/masterdataActions';
...
interface StatusFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialData: StatusTransaksi | null;
}
...
export default function StatusTransaksiFormModal({
  isOpen,
  onClose,
  initialData,
  ...
  const [state, formAction] = useActionState(
    initialData ? updateStatusTransaksi.bind(null, initialData.id) : createStatusTransaksi,
    { message: null, errors: {} }
  );
      ...
      <div className='relative w-full max-w-lg rounded-lg border bg-background shadow-lg'>
        <div className='flex flex-col space-y-1.5 p-6'>
          <h3 className='font-semibold tracking-tight text-2xl'>
            {initialData ? 'Edit Status Transaksi' : 'Tambah Status Transaksi Baru'}
          </h3>
