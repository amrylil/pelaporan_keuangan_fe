...
import TipeFormModal from './TipeTransaksiFormModal';
import { deleteTipeTransaksi } from '@/app/actions/masterdataActions';
...
export default function TipeClient({ data }: TipetProps) {
  ...
  const [editingData, setEditingData] = useState<TipeTransaksi | null>(null);
  ...
  const handleEdit = (tipe: TipeTransaksi) => {
    setEditingData(tipe);
    setIsModalOpen(true);
    setModalKey(Date.now());
  };
